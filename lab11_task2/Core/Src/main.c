...
/* Motor direction pins using your current setup on GPIOD */
...
/* safe max speed in percent */
uint8_t maxSpeedPercent = 60;
/* USER CODE END PV */
...
uint8_t PID_To_Speed(float value);
/* USER CODE END PFP */
...
void Motor_A_SetSpeed(uint8_t speed_percent)
{
  ...
  ccr_value = (speed_percent * 65535UL) / 100UL;
...
uint8_t PID_To_Speed(float value)
{
  ...
  /* simple scaling for beginner-style code */
  absValue = absValue * 1.5f;

  if (absValue > maxSpeedPercent)
  {
    absValue = maxSpeedPercent;
  }

  return (uint8_t)absValue;
}
...
int main(void)
{
  ...
  myPrintf("accAngle,gyroRate,angle,error,pidOutput\r\n");
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* gyro x-axis rate in deg/s */
    gyroRate = gx * 0.00875f;
    ...
    {
      uint8_t speedPercent = PID_To_Speed(pidOutput);

      if (pidOutput > 1.0f)
      {
        BOTH_MOTORS_FORWARD();
        Both_Motors_SetSpeed(speedPercent);
      }
    ...
    sprintf(uartBuf, "%.2f,%.2f,%.2f,%.2f,%.2f\r\n",
            accAngle, gyroRate, angle, error, pidOutput);
    myPrintf(uartBuf);
