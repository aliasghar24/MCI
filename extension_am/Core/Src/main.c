...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
  if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
    ...
    // Calculate period (handle basic rollover)
...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
        // MATH:
        // Clock = 48MHz. Prescaler = 47. 
        // Timer Clock = 48MHz / (47+1) = 1 MHz.
        ...
        // CHANGED: Transmit using huart2
        HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
    }
    
    HAL_Delay(100); // 100ms delay is sufficient
