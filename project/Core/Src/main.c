...
/* ---- IMU Calibration Offsets (User calibration) ---- */
static float gyro_offset = 0.0f;       // Gyroscope Y-axis offset (DPS)
static float acc_x_offset = 0.0f;      // Accelerometer X-axis offset (raw)
static float acc_z_offset = 0.0f;      // Accelerometer Z-axis offset (raw)
...
/* ============================================================
 *  IMU Calibration Function (User code)
 * ============================================================
 *  Collects 300 samples from gyroscope and accelerometer,
 *  averages them, and stores as offset values.
 *  Call this during initialization while the robot is stationary.
 * ============================================================ */
static void IMU_Calibrate(void) {
    ...
    char msg[50];
    int len = sprintf(msg, "Calibrating IMU...\r\n");
    ...
    for (int i = 0; i < CALIBRATION_SAMPLES; i++) {
        ...
        gyro_sum += (float)raw_gyro_y * 0.00875f;  // Convert to DPS

        /* Read Accelerometer X and Z axes */
        ...
        acc_x_sum += (float)acc_raw_x;
        acc_z_sum += (float)acc_raw_z;
    ...
    acc_z_offset = acc_z_sum / CALIBRATION_SAMPLES;

    // Display calibration results
    len = sprintf(msg, "Calib done: G=%.2f\r\n", gyro_offset);
    HAL_UART_Transmit(&huart2, (uint8_t*)msg, len, HAL_MAX_DELAY);
    HAL_Delay(100);
}
...
int main(void)
{
  ...
  /* Reset of all peripherals, Initializes the Flash interface 66666666666666666666.6.66.6666666and the Systick. */
  HAL_Init();
  ...
  // ---- IMU Calibration: Collect 300 samples to determine offsets ----
  // Keep robot stationary and level during this phase (~1.5 seconds)
  IMU_Calibrate();
  HAL_Delay(200);
  ...
  // Small delay before control begins
  HAL_Delay(100);
  ...
  while (1)
  {
      if (display_flag == 1) {
          ...
          char tx_buffer[100];
                int len = sprintf(tx_buffer, "%d,%d,%d,%d\r\n",
                  (int)(shared_angle * 100),
                  (int)(acc_angle * 100),
                  (int)(gyro_rate * 100),
                  (int)(shared_pid_out * 100));
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    ...
    if (htim->Instance == TIM2) {
        ...
        acc_raw_x -= (int16_t)acc_x_offset;
        acc_raw_z -= (int16_t)acc_z_offset;
